...
internal class AndroidReadoutPreferencesRepository(
    ...
    private fun enabledKey(simulator: String, label: String) =
        booleanPreferencesKey("${simulator}_${label}_enabled")
    ...
    override suspend fun saveReadoutEnabledState(simulator: String, label: String, enabled: Boolean) {
        dataStore.edit { it[enabledKey(simulator, label)] = enabled }
    }
    ...
    override suspend fun saveReadoutOrder(simulator: String, order: List<String>) {
        dataStore.edit { it[orderKey(simulator)] = order.joinToString(",") }
    }
}
