...
class ExitConfirmationPreferenceForExitTest {
    ...
    @Test
    fun `今後表示しないがオフなら終了確認設定を保存しない`() = runTest {
        var saveCallCount = 0
    ...
    @Test
    fun `今後表示しないがオンなら終了確認設定を無効で保存する`() = runTest {
    ...
    @Test
    fun `終了確認設定の保存がErrorを投げても終了処理を継続できる`() = runTest {
        var saveCallCount = 0
    ...
    @Test
    fun `終了確認設定の保存がキャンセルされた場合は再送出する`() = runTest {
