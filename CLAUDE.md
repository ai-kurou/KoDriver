...
また、ユニットテストを書ける実装コードを変更・追加した場合は、**完了報告の前に必ず対応するユニットテストを追加・更新すること**。ただし、Fake / Stub / Spy などのテストダブル、Koin などの DI Module、単純な Preview・サンプルデータ・定数定義のようにユニットテストの対象として意味が薄いコードは除外する。
