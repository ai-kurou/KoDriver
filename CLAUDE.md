...
│   └── data/       JVM データ層（JNA 共有メモリ）※ JVM 専用
├── app/
...
commonMain/presentation/
  ...
  LmuUiState.kt           LmuUiState（Connecting / Connected / Error）
  component/
    PlaceholderContent.kt PlaceholderContent
  readout/
    ReadoutScreen.kt      ReadoutContent, ReadoutListPane, ReadoutDetailPane
    LmuListItem.kt        LmuListItem
```
...
コードを変更・追加したら、**完了報告の前に必ず該当モジュールのユニットテストと detekt を実行すること**。

```bash
# 変更したモジュールのテストを実行（例: app:shared を変更した場合）
./gradlew :app:shared:jvmTest

# 変更したモジュールの detekt を実行
./gradlew :app:shared:detekt
...
テストが失敗・detekt で指摘がある場合は修正してからレポートする。
