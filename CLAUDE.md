...
- **ワークツリー・ブランチの片付けでは、ローカルブランチのみ削除し、リモートブランチは削除しないこと。** ユーザーから明示的に指示された場合のみリモートブランチを削除してよい。リモートブランチは CI のログや他の作業者のレビュー履歴から参照され続けることがあるため、マージ後の自動削除設定に委ねるか、ユーザーの判断に任せる。
