...
│   ├── readout-vehicle-damage/   車両故障アナウンス詳細 UI（VehicleDamageDetailPane）
│   ├── readout-flag/             フラグアナウンス詳細 UI（FlagDetailPane）
│   ├── narrator/                 音声再生エンジン（WavNarratorEngine, SoundPlayer, LmuNarratorViewModel）
│   └── other/                    その他画面（ライセンス一覧・詳細表示）
├── app/
...
### `feature:other` モジュール（KMP）

```
commonMain/
  OtherViewModel.kt      uiState: StateFlow<OtherListUiState> を公開
  OtherListUiState.kt    items, selectedItem
  OtherContent.kt        OtherContent（ListPane / DetailPane を組み合わせたルート Composable）
  OtherListPane.kt       その他項目の一覧と選択 UI
  OtherDetailPane.kt     戻る操作と詳細コンテンツを表示する共通コンテナ
```
...
commonMain/presentation/
  AppModules.kt           アプリ共通の Koin モジュール
  AppScreen.kt            AppDestination（enum）, AppScreen（ナビゲーション骨格）
  LicenseDetailPane.kt    OSS ライセンス一覧・詳細表示
  Color.kt                アプリ共通色
  Theme.kt                アプリ共通テーマ
```
...
6. デスクトップアプリの統合テスト（**常に実行すること**。Koin モジュール構成の変更は `AppTest` に影響するため）
...
./gradlew :feature:readout-vehicle-approach:verifyRoborazziJvmTest
./gradlew :feature:readout-vehicle-damage:verifyRoborazziJvmTest
./gradlew :feature:readout-flag:verifyRoborazziJvmTest
./gradlew :feature:other:verifyRoborazziJvmTest
./gradlew :app:shared:verifyRoborazziJvmTest

# 差分がある場合: ゴールデン画像を更新してからテストを再実行
./gradlew :feature:readout:recordRoborazziJvmTest
./gradlew :feature:readout:verifyRoborazziJvmTest
./gradlew :feature:readout-vehicle-approach:recordRoborazziJvmTest
./gradlew :feature:readout-vehicle-approach:verifyRoborazziJvmTest
./gradlew :feature:readout-vehicle-damage:recordRoborazziJvmTest
./gradlew :feature:readout-vehicle-damage:verifyRoborazziJvmTest
./gradlew :feature:readout-flag:recordRoborazziJvmTest
./gradlew :feature:readout-flag:verifyRoborazziJvmTest
./gradlew :feature:other:recordRoborazziJvmTest
./gradlew :feature:other:verifyRoborazziJvmTest
...
テストが失敗・detekt で指摘がある・assertModuleGraph で違反がある・スクリーンショットテストに差分がある・ビルドエラーがある場合は修正してからレポートする。
