...
- `release-apps.yml`: 手動でリリースする際に実行。まず `_e2e-android-maestro.yml`（`ref: main`）を実行し、成功した場合のみバージョンバンプ・MSI/APK ビルド・リリース作成に進む。バージョンバンプ後、`update-baseline-profile` ジョブが Android エミュレータ上で `./gradlew :app:androidApp:generateReleaseBaselineProfile` を実行して `app/androidApp/src/release/generated/baselineProfiles/baseline-prof.txt` を再生成し、差分があれば main へコミット・プッシュする（差分がなければコミットしない）。Android APK のビルド（`_build-android-release.yml`）はこの再生成後の main を対象に実行され、Windows MSI のビルド（`_build-windows-msi.yml`）はバージョンバンプ後すぐに並行して実行される
