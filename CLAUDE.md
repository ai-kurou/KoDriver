...
3. モジュールグラフの検証
4. スクリーンショットテストの検証（差分がある場合はローカルでゴールデン画像を更新せず、失敗内容を報告する）
...
# ゴールデン画像の撮影・更新は GitHub Actions で行う。
# ローカルでは recordRoborazziJvmTest を実行しないこと。
# UI 変更によって差分が発生した場合もゴールデン画像を更新せず、失敗したテストを報告すること。
