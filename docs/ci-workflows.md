...
- `_e2e-android-foldable-maestro.yml`: `_build-android-release.yml` で署名付き APK をビルドし、`reactivecircus/android-emulator-runner` の `profile: "7.6in Foldable"`（縦ヒンジの折りたたみ端末プロファイル）で起動したエミュレータ上で折りたたみ姿勢に応じた挙動を検証する E2E テスト。`.github/scripts/run-foldable-maestro-with-artifacts.sh` が `adb emu posture 3`（opened、ヒンジが完全に平らな姿勢）でアプリを起動し `.maestro/foldable-open-detail.yaml` で detailPane が開くことを確認したあと、`adb emu posture 2`（half-opened、ヒンジが平らに開いていない姿勢）に切り替えて `.maestro/foldable-detail-collapsed.yaml` で `ReadoutContent` の `windowPosture.shouldCollapseDetailPane`（`core/designsystem` の `TabletopPosture.kt`）により detailPane の選択が自動解除されることを確認する。`release-apps.yml` から呼び出されるほか、Actions の画面から `ref` を指定して手動実行できる
