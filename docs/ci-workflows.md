...
- `on-pull-request.yml`: PR 作成・更新時に静的解析・テストを実行（`detekt` ジョブとは別に、ktlint（コードスタイル）を検証する `ktlint` ジョブ = `./gradlew ktlintCheck` を実行）。同一 PR に新しいコミットが追加された場合は、実行中の古い CI をキャンセルする。`desktop-screenshot-test-verify` / `android-screenshot-test-verify`（`_screenshot-test-verify.yml` を呼び出す）は、まず verify を実行し、失敗した場合のみ golden 画像を再記録してコミット・プッシュしたうえでジョブを失敗させる。`android-maestro-e2e` に加えて `android-maestro-foldable-e2e`（`_e2e-android-foldable-maestro.yml` を呼び出す）も実行する
...
- `_e2e-android-foldable-maestro.yml`: `_build-android-release.yml` で署名付き APK をビルドし、`reactivecircus/android-emulator-runner` の `profile: "7.6in Foldable"`（縦ヒンジの折りたたみ端末プロファイル）で起動したエミュレータ上で折りたたみ姿勢に応じた挙動を検証する E2E テスト。`.github/scripts/run-foldable-maestro-with-artifacts.sh` が `adb shell cmd device_state print-states` の出力から `OPENED`/`HALF_OPENED` の state ID を動的に取得し（`adb emu posture` はエミュレータのセンサー層のみを制御し androidx.window の `FoldingFeature` に反映されないことがあるため、`adb shell cmd device_state state <id>` で DeviceStateManager を直接操作する）、`OPENED`（ヒンジが完全に平らな姿勢）でアプリを起動して `.maestro/foldable-open-detail.yaml` で detailPane 固有のテキスト（「閾値設定」。2ペイン表示では戻るボタンが出ないため、detailPane にしか存在しないテキストで判定する）が表示されることを確認したあと、`HALF_OPENED`（ヒンジが平らに開いていない姿勢）に切り替えて `.maestro/foldable-detail-collapsed.yaml` で `ReadoutContent` の `windowPosture.shouldCollapseDetailPane`（`core/designsystem` の `TabletopPosture.kt`）により detailPane の選択が自動解除され、そのテキストが表示されなくなることを確認する。`release-apps.yml` から呼び出されるほか、Actions の画面から `ref` を指定して手動実行できる
...
- `nightly-todo.yml`: 毎日 JST 午前5時ごろ（`workflow_dispatch` でも手動実行可）に起動する。JST の日付・曜日をシェル側で明示的に算出したうえで、Claude Code CLI（`CLAUDE_CODE_OAUTH_TOKEN` シークレットで認証、Pro/Max サブスクリプション枠を使用）に `docs/nightly-todo-list.md` の「毎晩実行する項目」とその曜日の「曜日ローテーション項目」を調査させ、`gh issue list` / `gh issue create` / `gh run list` / `gh api repos/*/actions/runs*`（GitHub Actions の実行履歴を読むための土曜ローテーション項目向け）の実行権限のみを与える（`Bash` ツールをこれらのコマンドに限定し、リポジトリ内ファイルの編集権限は与えない）。起票前には `gh issue list --search` でオープン・クローズ問わず同趣旨のIssueが既に存在しないか確認させ、重複して起票しないようにする。プロンプトはシェルのヒアドキュメント（`cat <<PROMPT`、クォートなし）で組み立てているため、本文中で `gh issue create ...` のようなコマンド例をバッククォートで囲む際は必ず `\`` とエスケープすること（エスケープを忘れると、ヒアドキュメント展開時にシェルがコマンド置換として実際に実行してしまい、`<タイトル>`/`<本文>` のようなプレースホルダーのまま Issue が起票される事故につながる。2026-09-04 に発生、#1416）。Claude Code の最終出力（各項目の確認内容・起票有無の判断理由）は `nightly-todo-summary.txt` に保存され、ワークフローのジョブサマリー（`$GITHUB_STEP_SUMMARY`）に転記される（改善案の起票が無かった夜でも、何を調査してなぜ起票しなかったかを確認できるようにするため）。Claude Code 実行後、ワークフロー側でリポジトリ内のファイルが変更されていないことを検証する（Issue起票のみを行う想定のため）。GitHub への操作にはデフォルトの `GITHUB_TOKEN`（`issues: write` 権限）を使用する（Issue作成はPR作成と異なりCIトリガーの制約を受けないため、`GH_PAT` は不要）
