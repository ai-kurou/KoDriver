...
- Zennで直近の記事を確認する場合、`WebSearch`はZennの新着記事のインデックス反映が遅く投稿直後の記事を拾えないことが分かっているため使わない。代わりに `WebFetch` で `https://zenn.dev/api/articles?order=latest&count=50` （Zennが公開している新着記事一覧のJSON API）を取得し、`published_at` が過去24時間以内でタイトルがAIコーディング・Compose Multiplatform・Android・Claude・Codex等に関連するものを絞り込む。該当記事があり、KoDriverに導入する価値があるとClaudeが判断したものがあれば improvement-ideas.md に追記する。
