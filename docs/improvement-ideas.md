...
- **対象**: `.github/workflows/on-pull-request.yml` / `on-main-merge.yml` の `desktop-screenshot-test` / `android-screenshot-test` ジョブ
  ...
  **追加の課題（record→verifyの直列化による所要時間増加）**: PR #566の分離後、`xxx-screenshot-test-verify` が `needs: xxx-screenshot-test-record` で直列依存するようになり、golden画像が実際に更新されるPRでは「record実行時間 + verify実行時間」を待つ必要があるため、分離前より`on-pull-request.yml`全体の所要時間が延びている（体感で10分未満→20分以上）。加えて、CIが差分を検知するたびに`chore: update golden images`を無条件に自動コミット・pushする現行方式は、意図しない見た目の変化もレビューされずにそのままゴールデン画像として確定してしまうリスクがある。
  **改善案（今後の検討）**: このプロジェクトは Compose Multiplatform で開発者はmacから開発しているが、フォントレンダリング等の差異により golden image は Linux CI 上でしか正しく生成できない（そのためローカルでの `recordRoborazziXxxTests` 実行はCI基準の golden と一致しない）という制約がある。この制約を踏まえ、record を「PRの push毎に自動実行」ではなく「PRコメントや `workflow_dispatch` などのオンデマンドトリガーでのみ実行」する方式への変更を検討する。通常のPR pushでは `verify` のみを他ジョブと並列実行し、golden画像が古ければ通常どおり失敗させる。開発者は失敗を確認した上で明示的にrecordジョブを起動し、CI(Linux)上で生成した画像を自動コミットさせる。これにより、(1) 生成環境はLinux CIのまま維持でき、(2) 通常のPRでは直列待ちが発生せず実行時間を短縮でき、(3) 意図しない見た目変化はverify失敗として検知でき自動コミットで握りつぶされない、という3点を同時に満たせる。
