...
| `:core:lmu-windows-rest-api-data` | LMU内蔵ローカルREST API利用（JVM）。天気予報（`/rest/sessions/weather`）のUseCase・Repositoryを実装済み | [図](graphs/core-lmu-windows-rest-api-data.svg) |
...
| `:feature:lmu-windows-readout-rain-detail` | 降雨アナウンスの詳細設定 UI（Narrator側の読み上げ判定ロジックへの配線は未実装） | [図](graphs/feature-lmu-windows-readout-rain-detail.svg) |
