...
target_issues_json="$(jq -c --argjson blocked "${blocked_issue_numbers}" --argjson limit "${MAX_ISSUES_PER_RUN}" \
  '[.[] | select(.number as $n | $blocked | index($n) | not)] | .[0:$limit]' \
  <<< "${all_issues}")"

target_issue_count="$(jq 'length' <<< "${target_issues_json}")"
if [ "${target_issue_count}" -eq 0 ]; then
  log "対象issueなし。終了します。"
  exit 0
fi

log "今回処理するissue数: ${target_issue_count}（上限 ${MAX_ISSUES_PER_RUN}）"

# process_issue implements the full single-issue nightly-implement flow
# (worktree creation, Claude Code invocation, preSubmitChecks, commit/push/PR
# creation, and cleanup on every exit path). Failures inside are handled by
# posting an issue comment and removing the label; the function itself never
# aborts the caller's loop over multiple issues.
process_issue() {
  local issue_json="$1"
  local issue_number issue_title issue_url branch_name worktree_dir
  ...
  local pre_comment_count
  pre_comment_count="$(gh issue view "${issue_number}" --repo "${REPO}" --json comments --jq '.comments | length')"

  local status_file commit_message_file pr_title_file pr_body_file
...
PROMPT
  ...
  (cd "${worktree_dir}" && git commit -F "${commit_message_file}")
  ...
  pr_url="$(cd "${worktree_dir}" && gh pr create --repo "${REPO}" --base main --head "${branch_name}" --title "$(cat "${pr_title_file}")" --body-file "${pr_body_file}")"
  pr_number="${pr_url##*/}"
...
issue_index=0
while [ "${issue_index}" -lt "${target_issue_count}" ]; do
  issue_json="$(jq -c ".[${issue_index}]" <<< "${target_issues_json}")"
