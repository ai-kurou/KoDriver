...
第一引数を「実装内容」、第二引数を「ベースブランチ」、第三引数を「CodeRabbitへのレビュー依頼有無（`true`/`false`）」として受け取り、作業用ワークツリーとブランチを作成したうえで実装に着手し、コミット・プッシュ・PR作成、CIの完了待ち、PRに来ている指摘の要約報告までを一気通貫で行ってください。
