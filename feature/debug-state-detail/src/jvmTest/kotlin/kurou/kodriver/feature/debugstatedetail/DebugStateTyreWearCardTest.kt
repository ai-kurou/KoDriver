...
class DebugStateTyreWearCardTest {
    ...
    fun `selectedSimulatorが未選択の場合は未取得の文言を表示する`() =
        composeScreenshotTest {
            ...
            onNodeWithText("タイヤ摩耗").assertIsDisplayed()
    ...
    fun `selectedSimulatorがLMUの場合は4輪の摩耗率をパーセントで表示する`() =
        composeScreenshotTest {
            ...
            onNodeWithText("タイヤ摩耗").assertIsDisplayed()
            onNodeWithText("FL 80.0%").assertIsDisplayed()
            onNodeWithText("FR 75.0%").assertIsDisplayed()
            onNodeWithText("RL 70.0%").assertIsDisplayed()
            onNodeWithText("RR 65.0%").assertIsDisplayed()
        }
