...
class TelemetryLogContentTest {
    ...
    fun `一覧ペインにログを表示する`() =
        composeScreenshotTest {
            ...
            onNodeWithText("フラッグ").assertExists()
            onNodeWithText("燃料残り周回数").assertExists()
            onNodeWithText("09:30:20.000 / レース +00:00:20.000").assertExists()
            val telemetryJson = """{"flag":"green","sector1":"clear","sector2":"clear","sector3":"clear"}"""
            onNodeWithText(telemetryJson).assertDoesNotExist()
        }
    ...
    fun `ログが0件の場合は空状態を表示する`() =
        composeScreenshotTest {
            ...
            onNodeWithText("ログはまだありません").assertExists()
            onNodeWithText("テレメトリを受信すると、ここに新しい順で表示されます。").assertExists()
        }
    ...
    fun `ログをタップするとdetailPaneにログIDを渡す`() =
        composeScreenshotTest {
            ...
            onNodeWithText("フラッグ").performClick()

            onNodeWithText("selected: 2").assertExists()
        }
    ...
    fun `readoutItemDisplayNameは既知の読み上げ項目IDを日本語名に変換する`() =
        composeScreenshotTest {
            ...
            expectedDisplayNames.groupingBy { it.second }.eachCount().forEach { (displayName, count) ->
                onAllNodesWithText(displayName).assertCountEquals(count)
            }
        }
    ...
    fun `データベースをリセット項目をタップすると確認ダイアログを表示する`() =
        composeScreenshotTest {
            ...
            onNodeWithText("データベースをリセット").performClick()
    ...
    fun `ログが0件の場合はデータベースをリセット項目を表示しない`() =
        composeScreenshotTest {
            ...
            onNodeWithText("データベースをリセット").assertDoesNotExist()
        }
    ...
    fun `確認ダイアログの実行ボタンをタップするとonResetConfirmを呼ぶ`() =
        composeScreenshotTest {
            ...
            onNodeWithText("削除する").performClick()
    ...
    fun `resetSucceededがtrueになるとスナックバーで成功を通知しconsumeする`() =
        composeScreenshotTest {
            ...
            onNodeWithText("データベースをリセットしました").assertExists()
            waitUntil { consumed }
        }
    ...
    fun `resetSucceededがfalseになるとスナックバーで失敗を通知する`() =
        composeScreenshotTest {
            ...
            onNodeWithText("データベースのリセットに失敗しました").assertExists()
        }
    ...
    fun `先頭から離れているときに新しいログが追加されると先頭へ戻るボタンを表示する`() =
        composeScreenshotTest {
            ...
            onNode(hasScrollAction()).performScrollToNode(hasText("オーバーヒート"))
            runOnIdle {
                logs.value =
                    listOf(createTelemetryLog(id = 100, readoutItemKey = ReadoutItemKey.LmuWindows.Flag.RedFlag)) +
                    logs.value
            }

            onNodeWithText("新しいログ").assertExists()

            onNodeWithText("新しいログ").performClick()

            onNodeWithText("レッドフラッグ").assertExists()
        }
    ...
    fun `scrollToTopRequestが増えるとリストを先頭へ戻す`() =
        composeScreenshotTest {
            ...
            waitUntil {
                onAllNodesWithText("タイヤ摩耗").fetchSemanticsNodes().isNotEmpty()
            }
        }
}
