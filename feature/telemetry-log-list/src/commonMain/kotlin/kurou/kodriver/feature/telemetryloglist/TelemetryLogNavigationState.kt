...
internal val TelemetryLogNavigationStateSaver: Saver<TelemetryLogNavigationState, String> =
    Saver(
        save = { it.current.name },
        restore = { name ->
            val destination = TelemetryLogPaneDestination.entries.find { it.name == name } ?: return@Saver null
            TelemetryLogNavigationState(NavBackStack(destination))
        },
    )
...
internal fun rememberTelemetryLogNavigationState(
    ...
    rememberSaveable(saver = TelemetryLogNavigationStateSaver) {
