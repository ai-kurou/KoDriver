...
class ReadoutContentTest {
    ...
    fun `lmu_windowsの詳細ペインに遷移後にbackHandlerのコールバックを呼ぶと一覧に戻る`() =
        composeScreenshotTest {
            ...
            onNodeWithText(tyreTemperatureText).assertExists()
    ...
    fun `tyre_wearの項目を選択すると詳細ペインのタイトルにタイヤ摩耗を表示する`() =
        composeScreenshotTest {
            ...
            onNodeWithText(tyreWearText).assertExists()
        }
    ...
    fun `バーチャルエナジー残量をタップすると選択コールバックを呼ぶ`() =
        composeScreenshotTest {
            ...
            onNodeWithText(veText).performClick()
            waitForIdle()
    ...
    fun `ace_windowsの燃料残量項目を選択すると詳細ペインのタイトルに燃料残量を表示する`() =
        composeScreenshotTest {
            ...
            onNodeWithText(remainingFuelText).assertExists()
        }
    ...
    fun `gt7_ps5の燃料残量をタップすると選択コールバックを呼ぶ`() =
        composeScreenshotTest {
            ...
            onNodeWithText(remainingFuelText).performClick()
            waitForIdle()
    ...
    fun `tyre_temperatureとその他の項目のSwitchはON_OFF変更コールバックを呼ぶ`() =
        composeScreenshotTest {
            ...
            onNodeWithText(tyreTemperatureText).assertExists()
            onAllNodes(hasSwitchRole()).assertCountEquals(2)
    ...
    fun `キュー追加トグルをクリックするとON_OFF変更コールバックを呼ぶ`() =
        composeScreenshotTest {
            ...
            onNodeWithText(tyreTemperatureText).assertExists()
            onAllNodes(hasQueueToggleRole()).assertCountEquals(2)
    ...
    fun `リストを下にスクロールすると先頭へ戻るボタンを表示して先頭へ戻れる`() =
        composeScreenshotTest {
            ...
            onNodeWithText(scrollToTopText).assertExists().performClick()

            waitUntil {
                onAllNodes(hasText(simulatorLabelText)).fetchSemanticsNodes().isNotEmpty()
            }
        }
    ...
    private fun DesktopComposeUiTest.assertAllItemsCanNavigateBack(
