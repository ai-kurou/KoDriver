...
class CheckAppUpdateAvailableUseCaseTest {
    ...
    fun `最新バージョンが現在と同じとき新バージョンなしと判定する`() = runBlocking {
        val useCase = CheckAppUpdateAvailableUseCase(FakeAppUpdateRepository(AppRelease("v1.0.0")))
    ...
    fun `最新バージョンがパッチだけ上のとき新バージョンありと判定する`() = runBlocking {
        val useCase = CheckAppUpdateAvailableUseCase(FakeAppUpdateRepository(AppRelease("v1.0.1")))
    ...
    fun `最新バージョンがマイナーだけ上のとき新バージョンありと判定する`() = runBlocking {
        val useCase = CheckAppUpdateAvailableUseCase(FakeAppUpdateRepository(AppRelease("v1.1.0")))
    ...
    fun `最新バージョンがメジャーだけ上のとき新バージョンありと判定する`() = runBlocking {
        val useCase = CheckAppUpdateAvailableUseCase(FakeAppUpdateRepository(AppRelease("v2.0.0")))
    ...
    fun `最新バージョンが現在より古いとき新バージョンなしと判定する`() = runBlocking {
        val useCase = CheckAppUpdateAvailableUseCase(FakeAppUpdateRepository(AppRelease("v0.9.9")))
    ...
    fun `タグにvプレフィックスがない場合でも正しく比較する`() = runBlocking {
        val useCase = CheckAppUpdateAvailableUseCase(FakeAppUpdateRepository(AppRelease("1.1.0")))
