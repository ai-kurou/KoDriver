...
data class LmuWindowsVehicleApproachData(
    ...
    /** 左側の最短横方向距離。並走していない場合は [Double.MAX_VALUE]。 */
    val lateralDistanceLeftMeters: LateralDistanceMeters,
    /** 右側の最短横方向距離。並走していない場合は [Double.MAX_VALUE]。 */
    val lateralDistanceRightMeters: LateralDistanceMeters,
) {
    /** 左側に並走車両が存在するかどうか。 */
