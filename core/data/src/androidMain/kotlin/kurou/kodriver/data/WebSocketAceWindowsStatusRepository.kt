...
internal class WebSocketAceWindowsStatusRepository(
    ...
    override fun statusStream(): Flow<AceWindowsStatusData> =
        serverIpRepository
            .serverIp()
            .flatMapLatest { ip ->
                if (ip == null) {
                    ...
                    client.webSocketFlow(
                        host = ip,
                        port = port,
                        path = KoDriverServerFeature.STATUS.webSocketPath(Simulator.AceWindows),
                        retryDelayMs = retryDelayMs,
                        decode = { json.decodeFromString<AceWindowsStatusData>(it) },
                    )
                }
            }
}
