...
internal class WebSocketAceWindowsRemainingFuelLapsRepository(
            ...
            .flatMapLatest { ip ->
                if (ip == null) {
                    ...
                    client.webSocketFlow(
                        host = ip,
                        port = port,
                        path = KoDriverServerFeature.REMAINING_FUEL_LAPS.webSocketPath(Simulator.AceWindows),
                        retryDelayMs = retryDelayMs,
                        decode = { json.decodeFromString<AceWindowsRemainingFuelLapsData>(it) },
                    )
                }
            }
}
