...
class WebSocketAceWindowsTyreCarcassTemperatureRepositoryTest {
    private lateinit var server: MockWebServer
    private lateinit var fakeIpRepository: FakeServerIpPreferencesRepositoryForAceTyreCarcassTemperature

    @BeforeTest
    fun setUp() {
        server = MockWebServer()
        server.start()
        fakeIpRepository = FakeServerIpPreferencesRepositoryForAceTyreCarcassTemperature(null)
    }
    ...
    private fun buildRepository(retryDelayMs: Long = 0L) =
        WebSocketAceWindowsTyreCarcassTemperatureRepository(
    ...
    fun `有効なJSONフレームを受信したときTyreCarcassTemperatureDataをemitする`() =
        runTest {
                ...
                MockResponse().withWebSocketUpgrade(
                    object : WebSocketListener() {
                        override fun onOpen(
                            ...
                            webSocket.send(ACE_TYRE_CARCASS_TEMPERATURE_JSON)
            ...
            assertEquals("/ws/ace_windows/tyre_carcass_temperature", server.takeRequest().path)
        }
...
private class FakeServerIpPreferencesRepositoryForAceTyreCarcassTemperature(
    initialIp: String?,
...
private val ACE_TYRE_CARCASS_TEMPERATURE_JSON =
    """
    {
        "wheels": {
            "FRONT_LEFT": 80.0,
