...
internal class FakeTelemetryRepository(
    private val connected: Boolean = true,
    private val stream: Flow<TelemetryData> = flowOf(),
) : TelemetryRepository {
    var disconnectCalled = false

    override fun telemetryStream(): Flow<TelemetryData> = stream
    override suspend fun isConnected(): Boolean = connected
    override suspend fun disconnect() { disconnectCalled = true }
}

internal fun fakeTelemetryData(speedX: Double = 0.0) = TelemetryData(
    timestampMs = 0L,
